import { STORYFREEZE_PREVIEW_STATE_GLOBAL, type StoryFreezePreviewStateV1 } from '../shared/preview-protocol.js';

export type PreviewStateTarget = typeof window & {
  [STORYFREEZE_PREVIEW_STATE_GLOBAL]?: StoryFreezePreviewStateV1;
  notifyPreviewStateChanged?: (status: StoryFreezePreviewStateV1['status']) => Promise<void>;
};
...
export function publishPreviewState(target: PreviewStateTarget, state: StoryFreezePreviewStateV1): void {
  ...
  try {
    ...
    void target.notifyPreviewStateChanged?.(state.status).catch(() => undefined);
  } catch {
