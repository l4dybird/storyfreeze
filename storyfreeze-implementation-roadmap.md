...
#### PR-534: Legacy viewport delay removal

StoryFreeze 9から継承した既定の`viewportDelay=300`は、viewport変更ごとに固定時間を追加する。PR-522以降はresource、metrics、font、image decode、double `requestAnimationFrame`による明示的な安定待機を持つため、既定値を`0`へ変更する。`--viewport-delay`自体は維持し、fixture固有の追加待機が必要な利用者は明示的に指定できる。

Storybook 10 static fixture、Playwright context、`parallel=4`、retry story除外の9 PNGをローカルで各3回測定した結果、平均wall timeは5,331 msから4,805 msへ9.9%短縮し、全3組でPNG pathとSHA-256が一致した。6/8並列、Playwrightでのowned reload省略、navigation時間へのdelay充当は有意な改善がないか悪化したため採用しない。
...
PR-540Aで役目を終えたPuppeteer/Playwright differential workflowを停止し、最終比較結果をhistorical recordとして保持する。PR-540Bで公開CLIと既存E2EをPlaywright専用化し、PR-540Cでadapter、依存、Chromium探索経路、比較専用scriptを削除する。以後の性能回帰はPlaywrightのprocess/context比較と既存E2Eで検証する。

- Puppeteer adapter削除
- `puppeteer` / `puppeteer-core` dependency削除
- `--browser-backend=puppeteer` 削除
- `puppeteerLaunchConfig` alias削除
- Puppeteer runtime型削除。77件の固定device registryはviewport互換契約として著作権表示付きで維持
- migration guide最終化
- dependency treeにPuppeteerがないことを検証

### 10.9 5G — 10x throughput tuning

1.0安定化へ進む前に、単一captureの微調整だけでなく、起動、queue、story/variant scheduling、browser topology、CI水平分割を含むfull-suite throughputを最適化する。10倍はcold同一runner、variant-heavy、warm CI、水平shardを混ぜずに個別評価し、正確性を弱めて達成した数値は採用しない。

Phase 6の実装stackは次の順で完了した。
...
## 11. Phase 6 — Playwright最適化・1.0安定化

Phase 6は新しいbrowser機能を増やす工程ではなく、公開契約、長時間安定性、Storybook 10互換、release経路を1.0品質へ固定する工程とする。既存のpackage smokeとStorybook 10 static E2Eを再利用し、新しいfixtureやsmoke suiteは追加しない。

Stacked PRは次の順で進める。

1. PR-600: package rootの公開TypeScript型、ESM export、CLI、TypeDoc entryを固定する
2. PR-601: 既存static E2Eを再利用したbrowser lifecycle soakとprocess/context leak判定を追加する
3. PR-602: 公開から24時間経過した最新Storybook 10へ既存React/Vite fixtureを更新する
4. PR-603: alpha、release candidate、stableのdist-tag、provenance、GitHub Releaseを明示的に検証する

RC直前の整理stackでは、公開型と内部依存方向、Previewのstory-session state、capture runtime policyを低リスクに分離した。その後、全production TypeScript、capture lifecycle、buffer/trace ownership、公開package、release workflowを最終レビューし、workspace内だけに残っていた脆弱なStorybook 10.5.0解決を10.5.2へ統一した。公開peer範囲`^10.0.0`、runtime dependency、CLI、PNG、既定の`process + strict`契約は変更していない。

RC readiness recordでは、packed presetをStorybook 10.0.0と10.4.0でloadし、現行10.5.2 fixtureのstatic E2Eを再利用した。alpha.9 masterとcleanup stack先端の既存PR-profile artifactを、同じAMD EPYC 7763、runner image、Chromium、Node、optionsの組で比較し、wall p50/p95、capture p95、RSS、CPUの回帰sanityをすべて満たした。詳細は`docs/rc-source-review.md`と`docs/rc-readiness.md`を正とする。

次工程はcleanup stackを順にmergeした後、最新`master`からChangesetsを`alpha`から`rc`へ移す独立PRだけである。古いRC transition draftは再利用せず、replacement PR作成後にsupersededとして閉じる。
...
| 600 | public package contract             | exports/types/docs |
